...
class Agent < Sequel::Model 
  ...
  def count_calls(bmonth, bday, byear, emonth, eday, eyear)
    Caction.filter('(timestamp >= ? and timestamp <= ?) and action = ? and agent_id = ?', Time.parse("#{bmonth}/#{bday} #{byear}").to_f, Time.parse("#{emonth}/#{eday} #{eyear} 23:59:59").to_f, "CONNECT", self.id).size
  end

  def talk_time(bmonth, bday, byear, emonth, eday, eyear)
    complete_calls = Caction.filter('(timestamp >= ? and timestamp <= ?) and (action = ? or action = ?) and agent_id = ?', Time.parse("#{bmonth}/#{bday} #{byear}").to_f, Time.parse("#{emonth}/#{eday} #{eyear} 23:59:59").to_f, "COMPLETECALLER", "COMPLETEAGENT", self.id)
    talk_time = 0
  ...
  def pause_time_per_day(month, day, year) # Pretty ugly, need to re-write this
    ...
    tmp_time = 0.0
    paused = false
    aactions = Aaction.filter('(timestamp >= ? and timestamp <= ?) and (action = ? or action = ?) and agent_id = ?', Time.parse("#{month}/#{day} #{year}").to_f, Time.parse("#{month}/#{day} #{year} 23:59:59").to_f, "PAUSE", "UNPAUSE", self.id)
  ...
  def login_time_per_day(month, day, year) 
    ...
    total_time = 0.0
    logged_in = false
    actions = Aaction.filter('agent_id = ? and timestamp >= ? and timestamp <= ?', self.id, Time.parse("#{month}/#{day} #{year}").to_f, Time.parse("#{month}/#{day} #{year} 23:59:59").to_f).sort_by { |action| action.timestamp }
    unless actions.empty?
      if actions.size > 1
