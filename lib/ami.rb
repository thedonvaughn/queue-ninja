...
class AMI
  ...
  def initialize(args = {})
    ...
    @server = @config['queueninja']['ast_server'] || "127.0.0.1"
    @port = @config['queueninja']['ast_port'] || "5038" 
    @username = @config['queueninja']['username'] || "username" 
    @secret = @config['queueninja']['secret'] || "secret"
