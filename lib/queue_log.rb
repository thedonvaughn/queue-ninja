...
class QueueLog
  attr_accessor :config, :queue_log, :new_records

  def initialize(args = {})
    @config = Queueninja.config
    @queue_log = @config['queueninja']['queue_log'] || "/var/log/asterisk/queue_log"
    @new_records = 0
  end
  ...
  def add_aaction(string)
    ...
    unless member[1] =~ /MANAGER|NONE/
      unless aaction = Aaction.find(:uniqueid => member[1].to_s)
      ...
      aaction = Aaction.filter('timestamp = ? and channel = ? and action = ?', member[0].to_f, member[3].to_s, member[4].to_s)
      if aaction.empty?
        queue = test_queue(member[2]) 
        agent = test_agent(member[3])
  ...
  def add_caction(string)
    ...
    unless call = Call.find(:uniqueid => caction[1].to_s)
    ...
    call.save
    cactions = Caction.filter('uniqueid = ? and action = ?', caction[1].to_s, caction[4].to_s)
    if cactions.empty?
  ...
  private
   ...
   def test_queue(qname)
    queue = Queu.find(:queue_name => qname.to_s)
    if queue.nil?
  ...
  def test_agent(channel)
    ...
    agent = Agent.find(:channel => channel.to_s)
    if agent.nil?
